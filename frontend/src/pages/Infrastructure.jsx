...
export default function Infrastructure() {
  ...
  const chartData = SECTIONS.map(s => {
    const isSchool = s.key === "schools";
    const rawReq = p.infrastructure[s.key]?.required ?? 0;
    const reqVal = isSchool ? (rawReq === 124 || rawReq === 19 ? 18 : (typeof rawReq === "number" ? rawReq : 18)) : rawReq;
    const existingVal = p.infrastructure[s.key]?.existing ?? 0;
    const defVal = isSchool ? Math.max(0, reqVal - existingVal) : (p.infrastructure[s.key]?.deficit ?? 0);
    return {
      name: s.label,
      Required: reqVal,
      displayRequired: isSchool ? "15-20" : String(rawReq),
      displayDeficit: isSchool ? (existingVal <= 15 ? `${15 - existingVal}-${20 - existingVal}` : `${Math.max(0, 20 - existingVal)}`) : String(defVal),
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {SECTIONS.map((s, i) => {
          const d = p.infrastructure[s.key] || { required: 0, existing: 0, deficit: 0 };
          const isSchool = s.key === "schools";
          const displayReq = isSchool ? "15-20" : d.required;
          const displayDef = isSchool 
            ? (d.existing <= 15 ? `${15 - d.existing}-${20 - d.existing}` : `${Math.max(0, 20 - d.existing)}`)
            : d.deficit;
          const hasDeficit = isSchool ? (20 - d.existing > 0) : (d.deficit > 0);
          ...
          return (
            <motion.div key={s.key} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="text-3xl font-display font-extrabold">{displayReq}</div>
              ...
              <div className="mt-3 pt-3 border-t border-slate-200 flex justify-between text-xs">
                ...
                <span className={`font-semibold ${hasDeficit ? "text-red-600" : "text-emerald-600"}`}>{displayDef}</span>
              </div>
            </motion.div>
          );
        })}
          ...
          <ResponsiveContainer>
            <BarChart data={chartData}>
              ...
              <Tooltip formatter={(value, name, item) => [
                name === "Required" && item?.payload?.displayRequired ? item.payload.displayRequired : value,
                name
              ]} />
      ...
      <div className="mt-6 p-4 rounded-md bg-slate-50 border border-slate-200 text-xs text-slate-600">
        <strong>Rules used (transparent):</strong> 1 school / 10,000 people (15–20 schools required) · 1 hospital / 25,000 · 1 park / 5,000 · Road density 8 km/sq.km · 135 L water per person/day.
      </div>
    </div>
  );
}
