...
class TestAutodesk:
    ...
    def test_connect_url_unauth_401(self):
        r = requests.post(f"{API}/autodesk/connect-url", json={"return_url": "https://example.com"}, timeout=15)
        assert r.status_code == 401

    def test_status_unauth_401(self):
        r = requests.get(f"{API}/autodesk/status", timeout=15)
        assert r.status_code == 401

    def test_disconnect_unauth_401(self):
        r = requests.post(f"{API}/autodesk/disconnect", timeout=15)
        assert r.status_code == 401

    def test_disconnect_authenticated(self, demo_headers):
        r = requests.post(f"{API}/autodesk/disconnect", headers=demo_headers, timeout=15)
        assert r.status_code == 200
        assert r.json().get("ok") is True

    def test_direct_projects_contents_not_connected_409(self, demo_headers):
        demo_user = db.users.find_one({"email": "demo@smartscape.ai"})
        if demo_user:
            db.aps_tokens.delete_one({"user_id": demo_user["id"]})
        r = requests.get(f"{API}/autodesk/projects/some-proj/contents",
                         headers=demo_headers, params={"folder_id": "root_folder_123"}, timeout=15)
        assert r.status_code == 409

    def test_direct_projects_contents_unauth_401(self):
        r = requests.get(f"{API}/autodesk/projects/some-proj/contents",
                         params={"folder_id": "root_folder_123"}, timeout=15)
        assert r.status_code == 401
...
class TestProjects:
    ...
    def test_real_time_risks(self, demo_headers):
        pid = TestProjects.project_id
        payload = {"lat": 18.5204, "lng": 73.8567, "location_name": "Test Pune Area"}
        r = requests.post(f"{API}/projects/{pid}/risks/real-time", headers=demo_headers, json=payload, timeout=30)
        assert r.status_code == 200
        data = r.json()
        assert data.get("data_source") == "real_osm"
        assert len(data.get("risks", [])) >= 6
        assert all("real_metrics" in item for item in data["risks"])
    ...
    def test_proposals_get(self, demo_headers):
        pid = TestProjects.project_id
        r = requests.get(f"{API}/projects/{pid}/proposals", headers=demo_headers, timeout=15)
        assert r.status_code == 200
        data = r.json()
        assert "winner_id" in data
        assert len(data.get("proposals", [])) >= 2
        assert len(data.get("head_to_head", [])) >= 8
        assert len(data.get("radar_data", [])) >= 6
        assert "recommendation" in data

    def test_proposals_compare_real_time(self, demo_headers):
        pid = TestProjects.project_id
        payload = {
            "sustainability": 50.0,
            "housing": 10.0,
            ...
            "mobility": 15.0,
            "economic": 10.0,
            "strategy_preset": "net_zero"
        }
        r = requests.post(f"{API}/projects/{pid}/proposals/compare", headers=demo_headers, json=payload, timeout=15)
        assert r.status_code == 200
        data = r.json()
        assert data.get("winner_id") == "P-B"
        assert data.get("margin") > 0
        assert data.get("strategy_preset") == "net_zero"

        # Select proposal
        r_sel = requests.post(f"{API}/projects/{pid}/proposals/select", headers=demo_headers, json={"proposal_id": "P-B"}, timeout=15)
        assert r_sel.status_code == 200
        assert r_sel.json().get("active_proposal_id") == "P-B"
...
class TestAutodeskLink:
    ...
    def test_link_unknown_project_404(self, demo_headers):
        demo_user = db.users.find_one({"email": "demo@smartscape.ai"})
        uid = demo_user["id"]
        db.aps_tokens.update_one({"user_id": uid}, {"$set": {
            "user_id": uid, "access_token": "bogus", "refresh_token": None,
            "expires_at": (datetime.now(timezone.utc) + timedelta(hours=1)).isoformat(),
            "scope": "data:read", "updated_at": datetime.now(timezone.utc).isoformat(),
        }}, upsert=True)
        try:
            r = requests.put(f"{API}/projects/nonexistent-pid/autodesk-link",
                             headers=demo_headers,
                             json={"hub_id": "b.hub-fake", "aps_project_id": "b.proj-fake"}, timeout=15)
            assert r.status_code == 404
        finally:
            db.aps_tokens.delete_one({"user_id": uid})

    def test_unlink_unauth_401(self, boundary_project):
        r = requests.delete(f"{API}/projects/{boundary_project}/autodesk-link", timeout=15)
        assert r.status_code == 401

    def test_contents_unknown_project_404(self, demo_headers):
        r = requests.get(f"{API}/projects/nonexistent-pid/autodesk-contents",
                         headers=demo_headers, timeout=15)
        assert r.status_code == 404
...
@pytest.fixture(scope="module")
def push_project(demo_headers):
    payload = {
        "name": "TEST_push_project",
        "location": {"name": "TestPushPlot", "lat": 18.5, "lng": 73.8},
        "site_area_sqkm": 10.0, "existing_population": 40000, "target_population": 90000,
        "growth_rate": 2.5, "planning_horizon_years": 15,
        "existing_schools": 5, "existing_hospitals": 1, "existing_parks": 3,
        "existing_roads_km": 30, "existing_buildings": 3000,
...
class TestAutodeskPush:
    def test_push_no_link_404(self, demo_headers, push_project):
        # ensure no autodesk link
        db.projects.update_one({"id": push_project}, {"$unset": {"autodesk": ""}})
        files = {"brief": ("brief.pdf", b"%PDF-1.4 fake", "application/pdf")}
        data = {"include_boundary": "true", "include_zoning": "true"}
        r = requests.post(f"{API}/projects/{push_project}/autodesk-push",
                        headers=demo_headers, files=files, data=data, timeout=30)
        assert r.status_code == 404

    def test_push_with_fake_token_graceful(self, demo_headers, push_project):
        demo_user = db.users.find_one({"email": "demo@smartscape.ai"})
        uid = demo_user["id"]
        ...
        # link the project
        requests.put(f"{API}/projects/{push_project}/autodesk-link",
                    headers=demo_headers,
                    json={"hub_id": "b.hub-fake", "hub_name": "FakeHub",
                          "aps_project_id": "b.proj-fake", "aps_project_name": "FakeProj",
                          "root_folder": "urn:adsk.wipprod:fs.folder:co.fake"}, timeout=15)
        try:
            ...
            proj = db.projects.find_one({"id": push_project})
            assert proj is not None and proj["name"] == "TEST_push_project"
        finally:
            db.aps_tokens.delete_one({"user_id": uid})
            requests.delete(f"{API}/projects/{push_project}/autodesk-link",
                           headers=demo_headers, timeout=15)

    def test_push_nothing_to_upload_400(self, demo_headers, push_project):
        demo_user = db.users.find_one({"email": "demo@smartscape.ai"})
        uid = demo_user["id"]
        db.aps_tokens.update_one({"user_id": uid}, {"$set": {
            "user_id": uid, "access_token": "bogus", "refresh_token": None,
        ...
        requests.delete(f"{API}/projects/{push_project}/boundary",
                       headers=demo_headers, timeout=15)
        requests.put(f"{API}/projects/{push_project}/autodesk-link",
                    headers=demo_headers,
    ...
    def test_push_unknown_project_404(self, demo_headers):
        r = requests.post(f"{API}/projects/nonexistent-pid/autodesk-push",
                          headers=demo_headers, data={"include_boundary": "true"}, timeout=15)
        assert r.status_code == 404

    def test_push_unauth_401(self):
        r = requests.post(f"{API}/projects/some-id/autodesk-push",
                          data={"include_boundary": "true"}, timeout=15)
        assert r.status_code == 401
...
# ---------- Real Geospatial Layer (Priority 3) ----------
class TestRealGeodata:
    def test_real_suitability_score_success(self, demo_headers, boundary_project):
        r = requests.post(f"{API}/projects/{boundary_project}/real-suitability-score",
                          headers=demo_headers, timeout=45)
        assert r.status_code == 200, r.text
        j = r.json()

        # Check data_source tags
        assert j.get("data_source") == "real", "Root data_source must be 'real'"
        assert j.get("score", {}).get("data_source") == "real", "Score data_source must be 'real'"
        assert j.get("sustainability", {}).get("data_source") == "real", "Sustainability data_source must be 'real'"

        # Check overall score and breakdown
        overall = j.get("overall")
        assert overall is not None and 0 <= overall <= 100
        breakdown = j.get("breakdown")
        assert isinstance(breakdown, list) and len(breakdown) == 6

        categories = [b["category"] for b in breakdown]
        expected_cats = [
            "Infrastructure Fulfillment", "Land Use Efficiency", "Accessibility",
            "Green / Open Space", "Sustainability", "Population Capacity"
        ]
        for cat in expected_cats:
            assert cat in categories, f"Missing category {cat} in breakdown"

        # Check amenities
        amenities = j.get("amenities")
        assert amenities is not None
        for key in ["schools", "hospitals", "parks", "roads"]:
            assert key in amenities, f"Missing amenity {key}"
            assert "nearest_distance_m" in amenities[key]
            assert "count" in amenities[key]

        # Verify persisted in database
        proj = db.projects.find_one({"id": boundary_project})
        assert proj.get("real_suitability") is not None
        assert proj["real_suitability"]["data_source"] == "real"

    def test_real_suitability_score_unknown_project_404(self, demo_headers):
        r = requests.post(f"{API}/projects/nonexistent-project-id/real-suitability-score",
                          headers=demo_headers, timeout=15)
        assert r.status_code == 404

    def test_real_suitability_score_unauth_401(self, boundary_project):
        r = requests.post(f"{API}/projects/{boundary_project}/real-suitability-score", timeout=15)
        assert r.status_code == 401

    def test_forma_massing_endpoint(self, demo_headers, boundary_project):
        r = requests.get(f"{API}/projects/{boundary_project}/autodesk-forma/massing?density_factor=1.1",
                         headers=demo_headers, timeout=15)
        assert r.status_code == 200
        data = r.json()
        assert "massing" in data and "geojson" in data
        massing = data["massing"]
        assert "parcels" in massing
        assert "summary" in massing
        assert massing["summary"]["parcel_count"] > 0
        assert massing["summary"]["total_gfa_sqm"] > 0
        # Check first parcel properties
        p0 = massing["parcels"][0]
        assert "height_m" in p0 and p0["height_m"] > 0
        assert "stories" in p0 and p0["stories"] >= 1
        assert "far" in p0
        assert "footprint_sqm" in p0 and p0["footprint_sqm"] > 0
        assert "typology" in p0

    def test_forma_export_obj_endpoint(self, demo_headers, boundary_project):
        r = requests.get(f"{API}/projects/{boundary_project}/autodesk-forma/export-obj",
                         headers=demo_headers, timeout=15)
        assert r.status_code == 200
    ...
    def test_forma_simulate_microclimate_endpoint(self, demo_headers, boundary_project):
        payload = {
        ...
        r = requests.post(f"{API}/projects/{boundary_project}/autodesk-forma/simulate-microclimate",
                          headers=demo_headers, json=payload, timeout=15)
        assert r.status_code == 200
        data = r.json()
        assert "forma_analysis" in data
        assert "sustainability" in data
        fa = data["forma_analysis"]
        assert "solar" in fa and fa["solar"]["mean_daily_sun_hours"] > 0
        assert "wind" in fa and "lawson_comfort_distribution" in fa["wind"]
        assert "daylight" in fa and fa["daylight"]["mean_sda_pct"] > 0
        assert "carbon" in fa and fa["carbon"]["total_embodied_carbon_tonnes"] > 0
        assert "acoustic_noise" in fa

        # Check DB update
        proj = db.projects.find_one({"id": boundary_project})
        assert proj.get("forma_analysis") is not None
        assert proj.get("sustainability", {}).get("data_source") == "forma_live"

    def test_forma_endpoints_unknown_project_404(self, demo_headers):
        r1 = requests.get(f"{API}/projects/unknown-proj/autodesk-forma/massing", headers=demo_headers, timeout=15)
        assert r1.status_code == 404
        r2 = requests.get(f"{API}/projects/unknown-proj/autodesk-forma/export-obj", headers=demo_headers, timeout=15)
        assert r2.status_code == 404
        r3 = requests.post(f"{API}/projects/unknown-proj/autodesk-forma/simulate-microclimate", headers=demo_headers, timeout=15)
        assert r3.status_code == 404

    def test_project_geojson_export(self, demo_headers, boundary_project):
        r = requests.get(f"{API}/projects/{boundary_project}/geojson", headers=demo_headers, timeout=15)
        assert r.status_code == 200
        data = r.json()
        ...
        b_feat = next((f for f in features if f.get("properties", {}).get("feature_type") == "project_boundary"), None)
        assert b_feat is not None
        assert b_feat["geometry"]["type"] == "Polygon"
        # Check zone feature
        z_feat = next((f for f in features if f.get("properties", {}).get("feature_type") == "urban_zone"), None)
        assert z_feat is not None
        assert "far" in z_feat["properties"]
        assert "area_hectares" in z_feat["properties"]
